...
//                        for i = 1 to 10 do
//                            state <- (BasicWorldStateDelta.ApplyDelta state delta).Value) numIter
//        printfn "Apply delta 10 times (immutable): %.3f" t
...
//                        let mutable state = startState
//                        for i = 1 to 10 do
//                            state <- (BasicWorldStateDelta.ApplyDelta3 state delta).Value) numIter
//        printfn "Apply delta 10 times (mutable): %.3f" t
//
        let t = time (fun () -> 
            let grid = GridStateTracker Seq.empty
            let robot:BasicRobot = {Position=IntVec3.Zero; Direction=IntVec3.UnitZ}
        ...
        let t = time (fun () -> 
            ...
            Simulator.RunOptimized37 program importedModules Debugger.stateFunctionsNoOp cache initState |> ignore) numIter
        printfn "Simulate optimized (no cache): %.3f" t
        let t = time (fun () -> 
            let cache = Simulator.Dict ()
            let robot:BasicRobot = {Position=IntVec3.Zero; Direction=IntVec3.UnitZ}
            let initState: BasicWorldState2 = {Robot=robot; Grid=Map.empty}
            Simulator.RunOptimized37 program importedModules Debugger.stateFunctions cache initState |> ignore) numIter
        printfn "Simulate optimized (immutable cache): %.3f" t
        let t = time (fun () -> 
            let cache = Simulator.Dict ()
            let robot:BasicRobot = {Position=IntVec3.Zero; Direction=IntVec3.UnitZ}
            let initState: BasicWorldState3 = {Robot=robot; Grid=System.Collections.Generic.Dictionary()}
            Simulator.RunOptimized37 program importedModules Debugger.stateFunctions2 cache initState |> ignore) numIter
        printfn "Simulate optimized (mutable cache): %.3f" t
