...
type CachingWorkshopDebugger3 (init: DebuggerInitialData, maxSteps) =
    let nie () = raise (System.NotImplementedException ())

    let cache = Simulator.MutableDict ()
    let newSimulator () : IGridWorldSimulator<_,_> = upcast DeltaRobotSimulator3 (init.State.Grid, init.State.Robot)
    let mutable simulator = newSimulator ()
