...
We compared the caching method to two baselines: normal simulation of the pogram using both a hashtable and tree map for the world state data structure. We use the hashtable for the primary baseline since it outperforms the tree map on all programs. The results as summarized in Figure~\todo{SDFasdfasdf}. The general trend is that our optimized technique performs slightly more poorly for small programs, but vastly outperforms the baseline for the larger programs.
